# Consumption journey — buying, using, adjusting, renewing

Status: desk-research hypothesis. Play: *Map the Consumption Journey* — how the offering is discovered, adopted, and kept; distinct from the [job map](job-map.md), which models the job itself.

realize need → explore options → configure & negotiate → commit → install & onboard → operate → adjust → renew or exit

## Product frame vs service frame
...
- At *explore*: comparing a capital purchase against a service subscription is genuinely hard.
- At *configure*: the articulation barrier ([persona-design-specifier.md](persona-design-specifier.md)).
- At *adjust*: fear of lock-in — opaque change costs are the service-model version of vendor lock.

## Related

- [persona-building-operator.md](persona-building-operator.md) — owns the back half of this journey
- [job-stories.md](job-stories.md) — stories 2 and 4 live at *adjust* and *renew*
