...
| [A choice that carries its words can be revisited without re-litigation](assertions/choices-carry-their-words.md): grounds at the level of quoted words are what keep delegation tolerable | The document channel implements it (RFQ clause + quote, verified in app); tender compliance practice cites clauses; the prose channel is untested — the [choice-provenance spec](../specs/choice-provenance/requirements.md) exists to test it |
