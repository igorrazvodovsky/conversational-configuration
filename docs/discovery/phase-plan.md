...
| 3 | Sketch two alternative comparison layouts against [trade-offs shown as a pair](principles/trade-offs-shown-as-a-pair.md) and pick one. Needed footprint in the model first, since a one-objective comparison cannot test it — unblocked since the [footprint spec](../specs/environmental-footprint/requirements.md) landed 2026-08-13 | Exploring → focusing | Comparison model. Its state-model half is answered ahead of it: [parallel-drafts](../specs/parallel-drafts/requirements.md) settled several live drafts over one configuration with history 2026-08-16 and shipped them 2026-08-17, so the layout has two full documents to arrange rather than a document and a flat snapshot, and a sketch can now be tested against real ones in the app. The layout itself is untouched, and it is the half this task exists for |
| 4 | Judge the [demo scenarios](../specs/demo-scenarios/requirements.md) against the principle coverage table ([direction.md](direction.md) §4); add or amend a scenario to cover [configuration can start from any variable, in any order](principles/start-from-any-variable.md) | Focusing | Done 2026-08-13 — scenario 4 recast as renewal-as-revision opening from a bare shaft-dimension statement; scenario 1 gained provenance + one-turn override; scenario 3's footprint delta registered as a deferred assertion. [Demo scenarios](../specs/demo-scenarios/requirements.md) ready for approval |
...
Current read: continue. The exploration gap in §1 is narrowing but unresolved on the side that matters. The comparison view is undrawn: footprint's data exists and [parallel-drafts](../specs/parallel-drafts/requirements.md) has settled what is being compared, so the two-objective layout can be judged against [trade-offs shown as a pair](principles/trade-offs-shown-as-a-pair.md) with nothing left blocking it, but no alternatives have been sketched. That the state model was decided in argument, from a spec conversation rather than from a drawing, is itself the §1 diagnosis repeating: mechanism gets its alternatives, presentation does not. Task 10 is the first move against that habit — the viewpoint set was chosen from three sketched candidates — but it settles a new surface rather than the comparison layout, which stays undrawn. The session map has not been started. And the first proposal's objective is settled only as a disclosed default ([models/Conversation moves.md](models/Conversation%20moves.md) §6); whether it should instead arrive as a pair waits on that comparison model.
