...
| *Revision* — find the term, change it | The operator, the primary persona | Random access; tables are good at this, prose is terrible at it |

Constitution #3 makes multiplying projections nearly free — every surface is computed from one solver-owned state. But [the canvas is the durable state](../assertions/canvas-is-the-durable-state.md) requires *one* durable locus; the resolution has to layer the readings inside one artifact, not multiply artifacts.

## 2. The option field

| Option | What it would mean here | Verdict |
|---|---|---|
| Spec sheet (as built) | Groups in model order, one row per variable, agreement header on top | Demoted to a layer. As the whole artifact it fails twice: the record answers in catalogue terms — the articulation barrier removed at input re-enters at the primary surface ([speak the building's language](../principles/speak-the-buildings-language.md), output side) — and a sheet of labeled rows *performs interrogation*: it reads as a questionnaire someone pre-filled, undercutting the genre half of [a candidate beats a question sequence](../assertions/candidate-beats-questions.md) |
| Several parallel canvases, one per reading, switchable | A quote view, a story view, a table view over the same state | Set aside. Dilutes the durable locus and reopens "which view is the record?" — the failure [the canvas is the durable state](../assertions/canvas-is-the-durable-state.md) names |
| Narrative-only document | The agreement as continuous prose | Set aside. Serves first-read comprehension, fails random access — and the primary persona is a reviser ([problem-framing.md](../problem-framing.md) §3) |
...
The layering is corroborated from outside: the [rfq-reconciliation spec](../../specs/rfq-reconciliation/requirements.md)'s stage-1 vocabulary — `agreement`, `context`, `performance` only — is exactly the recitals and operative-terms layers, drawn independently. A stage-1 RFQ is this document with no schedules: the customer's half of the genre pair, and its deviation register is the join between the two documents (a compliance matrix). Stage 2 begins precisely where a document enters the schedule layer. On a document-genre canvas, deviations render as margin marks on the affected terms — the seam is stated in both specs.
...
- Revision sessions go straight to the expanded schedules and never touch the terms layer → the terms altitude is wrong and the sheet was the honest genre after all.
...
- Principles drawn against: [speak the building's language](../principles/speak-the-buildings-language.md), [the canvas remembers, the chat explains](../principles/canvas-remembers-chat-explains.md), [any door is an entrance](../principles/any-door-is-an-entrance.md), [the agent proposes, the user disposes](../principles/agent-proposes-user-disposes.md)
- Assertions put under test: [the representation of the agreement selects the user's moves](../assertions/representation-selects-moves.md), [the canvas is the durable state](../assertions/canvas-is-the-durable-state.md)
