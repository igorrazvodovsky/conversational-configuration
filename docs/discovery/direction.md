...
| Canvas anatomy — outcome terms, derived hardware, price/footprint, status vocabulary | How does one screen show a whole agreement plus its validity? | Built by the [canvas](../specs/configuration-canvas/requirements.md) and reframed by the [service-agreement spec](../specs/service-agreement/requirements.md): outcome terms now sit above the derived hardware on the live canvas. The mechanisms were chosen against named alternatives (edit protocol, control vocabulary, control-selection locus — the [canvas design](../specs/configuration-canvas/design.md)). The anatomy itself is undrawn, and what is built predates the footprint frame |
