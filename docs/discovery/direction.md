...
| Comparison view — two drafts, differing variables, two deltas                        | How is a two-objective trade-off read at a glance?                      | Chat-side diff built and carrying both deltas since the [footprint spec](../specs/environmental-footprint/requirements.md) landed; what it compares becomes two live drafts under [parallel-drafts](../specs/parallel-drafts/requirements.md). Both the *layout* and the canvas placement stay unexplored — no alternatives yet sketched against [trade-offs shown as a pair](principles/trade-offs-shown-as-a-pair.md)                                                                                                                                                             |
...
The outstanding *exploring* work is concentrated in two rows — the comparison view, judgeable against [trade-offs shown as a pair](principles/trade-offs-shown-as-a-pair.md) now that footprint exists and now that it has two real documents to lay out rather than a document and a snapshot, but still undrawn, and the session map, which has no counterpart in code either. That is the finding in [phase-plan.md](phase-plan.md) §1.
