...
*Test.* Can the user always tell who chose a value, and undo it in one move? The first half is answered by the provenance tag being visible on the canvas rather than recorded in the transcript ([../models/Conversation moves.md](../models/Conversation%20moves.md) §1). The undo half is unbuilt, and is seeded as the [undo spec](../../specs/undo/requirements.md).
