...
Five walkthroughs of the running prototype, one per scenario in [discovery/scenarios/](discovery/scenarios/), where each is grounded in a job story and mapped to the principles it demonstrates and the assertions it exercises ([discovery/direction.md](discovery/direction.md) §4). Each is also an automated check: `cd agent && uv run --env-file ../.env pytest -m scenario` drives the same turns against the agent graph and asserts the same outcomes ([conversation checks](specs/conversation-checks/requirements.md)). This document and the harness are two performances of the same notes; when they disagree, one of them has drifted from its note.

Run the app with `npm run dev` (UI on 3000, agent on 8123). Every scenario starts from *New elevator* on the `/` list, which opens a workspace with an empty agreement, unless the scenario says otherwise. Nothing carries over between scenarios; a scenario left half-finished is easiest to abandon by starting a new elevator rather than by undoing.

Two things to say once, at the start of the first scenario, so they do not have to be repeated: the canvas on the left is the agreement itself in three layers — recitals, numbered operative terms, collapsed schedules — and it is the durable record; the chat on the right is where the consequences are explained. Every price in the system is a monthly service fee, never a purchase price.

## Reading the steps

Each turn is either something typed into the composer, or a click on a control. Typed turns are given verbatim; the agent's wording back will vary and the script never depends on it. What to point at is what the solver produced, which does not vary: a value on the sheet, a card, a rule label, a figure.

Prices are quoted where the choice set that produces them is pinned by the walkthrough. Where the agent has latitude about what else it records, the fee moves and the script says so rather than naming a figure.
...
*Setup.* A new elevator. The *Hospital bed lift* entry chip is the shortest way in; the walkthrough below types its own opening so the building is stated in one turn.
...
2. Whatever the agent asks next, answer it in ordinary words, or click the chips it offers. Do not type a value code at any point; the scenario's whole claim is that none is needed.

3. Type: *You choose the cabin — the wall finish and the floor — whatever suits a ward block that gets cleaned every night.*

   The agent picks. Hover the provenance mark beside the car wall finish (*why this value is here*) and show that it reads as the agent's choice, not the customer's.
...
*The claim.* Changing something already decided is an ordinary move. When the change collides with earlier decisions, the system does not refuse — it computes the ways forward, names the rules behind each, and lets the customer pick or walk away.
...
3. Click *Keep everything as it is — abandon this change*.
   ...
   Nothing on the sheet moves. This is worth showing before applying the repair, because it is the half of the interaction that a compliance-checking tool does not have.
...
3. Type: *Keep this one as the practical option — I'd like to see a premium version beside it without losing this.*
   ...
   The agent forks rather than revising, and names the new draft itself from the conversation. Point at the canvas head: the draft's name is now beside the agreement's, with a chevron. Nothing was asked about the name, and the agent did not announce it.
...
7. Open the drafts menu again and switch back to *Original*.
   ...
   The whole document changes back — its own values, its own provenance, its own undo history. The premium draft is untouched and still in the menu with its own price.
...
1. With the workspace open, reload the browser.
   ...
   The draft name in the canvas head arrives a tick after the rest of the page — it is drawn only once the client has mounted, because the drafts it names exist only on the client. Wait for it rather than reloading again.

2. Type: *Where were we, and what's still open on this one?*

   The answer comes from the agreement, not from the transcript above it — which matters because another conversation may have moved it since. Point at the answer naming what is undecided; that list is computed from the agreement and was never stated in the conversation.

3. Type: *The shaft is 1800 by 1700.*

   A bare dimension, with no control clicked and no pending question to answer. It lands on the shaft as the customer's own choice. If it collides with what is already recorded — it will if the car is large enough — the repair card of walkthrough 2 appears here instead, with its rules, and the change is still a revision rather than a restart.
...
*The claim.* The customer's own document is a way in. The system seeds the agreement from it, each term carrying its clause, and reports what it cannot meet as a named deviation with the rules behind it — a negotiating position rather than a compliance verdict.
...
1. Type a sentence of framing and paste the document under it: *We've issued an RFQ for the lift package. Here it is in full — can you tell me what you can do against it?*
   ...
   The agent does not start eliciting. It reads the catalogue, ingests the document once, and the sheet fills from it. Point at the provenance marks: every seeded term reads as the document's, with its clause number.

   Check before going on that a deviation appeared. The agent occasionally fails to map clause 1.2, the one that keeps the existing shaft, and without it there is no conflict to show and the reply says every requirement is met. If that happens, start a new elevator and paste again rather than improvising — steps 2 to 4 have nothing to stand on.

2. Point at the deviation on the sheet, under the rated speed: *Your document asked 3.0 m/s*, against the 2.5 m/s the agreement offers. The rules named are R04, which ties speed to minimum headroom, and R28, which says a modernization cannot raise the headroom of an existing shaft to 4600 mm. Clause 3.1 asked for the speed; clause 1.2 said the shaft is not being altered; the two cannot both hold, and the document's own author is the person who can decide which gives.
...
4. Click *Accept 2.5 m/s* on the deviation.
   ...
   The requirement is waived, and stays on the sheet marked *waived, still listed*. A waived requirement is answered, not forgotten — which is the difference between this and a compliance matrix where the row disappears.

5. Revise a different requirement the agreement already meets — for instance the contract term or the accessibility package — by opening that term's editor on the sheet and picking another value.

   A requirement the agreement meets carries no margin mark and no *Accept* button; there is nothing to answer. What it does carry is invisible until the click: because the document speaks to that term, the editor dispatches a reconciliation rather than the ordinary hidden canvas edit, so the change appears in the chat as a sentence instead of passing silently. Point that out — the same control, on the same sheet, means something different on a term the customer's own document named.
