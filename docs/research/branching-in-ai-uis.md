...
- *The measured cost of parallel scenarios is the cost of controlling them.* Lunzer and Hornbæk's first study got satisfaction and no time saving, because setting up and steering the scenarios ate it. The spec's controls are two structured sentences and a switcher in the canvas head, which is about as cheap as the mechanism gets, and the [design's](../specs/parallel-drafts/design.md) reading of proliferation — that if drafts accumulate unread the affordance is wrong rather than the count — is the right thing to watch during the next prototype walkthrough ([phase-plan](../discovery/phase-plan.md) §3).
