...
17. *A control is not disabled to mean no.* Disabling says that a gesture is unavailable and says nothing about why, in a channel that carries no words, at low contrast, outside the tab order. So the control stays live, the click happens, and what the interface knows is said back — in the transient channel `src/lib/say-why.ts` owns, or, where the click has a real action behind it, by taking it. A ruled-out option is the case that matters most: the rules that separate it from the agreement are computed, the repairs that would admit it are computed too, and asking for it anyway is how a customer reaches them. Two things are still disabled, and neither is a refusal. A spent card's controls are disabled because the turn they belong to is over and the card is now the transcript's record of it, not a control. A destructive or non-idempotent action guards itself in its handler and says in words that it is working; the attribute would be a weaker guard and a worse sentence. Anything else that greys itself is a reason the interface has and is not saying.
