...
`vitest.config.mts` now declares `offline` (node) and `interface` (jsdom) as
...
- `server.deps.inline` for `@copilotkit` and `@ag-ui`. The packages import their
  own stylesheet, and Node cannot load a `.css` file; inlining them makes Vite
  do the transform.
...
`testTimeout` is 30s here against the 5s default. Attaching runs several awaited
round trips before it settles. The checks finish in about a second each; the
ceiling exists to keep a genuine hang legible rather than to accommodate a slow
one.
...
that is the point of checking it. So a recording could never have supplied the
...
own fixtures against them (offline-checks design §5). The interface checks take
...
project, so `npm test` now runs it twice, and this tier inherits the offline
tier's standing condition that a frontend-only checkout cannot run it.
...
why a healthy agent does not produce it. Without that, a reader cannot tell a
...
That the checks discriminate was verified the way offline-checks §5 verified its
own fixtures, by breaking the thing they watch. With the hook's
...
One thing that cannot be asserted, and the attempt is worth recording. A check
that watches the wipe *appear* in the DOM before asserting the recovery fails,
...
negative control is what stands in for it.

The agreements are chosen by name, not by position in the dump. `chosen` and
`priced` carry identical choices — pricing lands in a candidate rather than in
the choices — so a pair drawn from those two would have passed whichever draft
won. The pair is `chosen` and `seeded`, and a check asserts up front that the
two differ, so a future dump that collapses them fails loudly instead of going
quiet.
...
- `npm test`: 194 passed across 8 files, ~6s — 187 in `offline`, 7 in
  `interface`. `AGUIMock` is started in `beforeAll`; a failure to start it surfaces
  as vitest's own suite-level error, naming the cause, rather than as failing
  assertions.
...
- CI needed no new step: the frontend job already runs `npm test` and already
  provisions the agent, which both tiers now need. Its comments were corrected
  to say why.

## Known gaps

- The orderings are composed from a reading of the hook's own design. A trap the
  design does not name is a trap this tier does not check, and the only thing
  that would find one is the running app.
- No check here renders the canvas, the chat pane, or any component that owns
  ...
  scroll position, `inert`-versus-`display:none`, and the WebGL render remain
...
- `card-dispatch.ts` is still unchecked. Its staleness rule is exercised
  ...
  calls, which offline-checks design §2 already declined to check.
