...
- One addition beyond the draft: candidates and frames also carry an optional `objective` key ("price" / "co2"). The canvas header subtitle names the candidate's objective ([agreement-document design](../agreement-document/design.md)), and with two objectives a hardcoded "cheapest completion" would mislabel a lowest-footprint candidate. Absent on pre-footprint threads, which were always cheapest.
