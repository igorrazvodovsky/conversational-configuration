...
- *Cheapest-vs-greenest side by side* needs no new tool: `propose_completion()` → `save_frame("Cheapest")` → `propose_completion(objective="co2")` → `compare_frames("Cheapest")` — the existing frames machinery is exactly this flow. Per [where guidance lives](../agent-tools/design.md), the four-step sequence is taught by `propose_completion`'s docstring and its conversational trigger — offer the pair when the customer signals footprint interest — by the system prompt.
