...
Three additions. Prefer `revise_choices` over `set_choices` whenever the customer changes an already-recorded decision. When repairs come back, present them as choices rather than verdicts. And offer a fork before big exploratory changes — "want me to keep this one and try that on a second draft?" — which was `save_frame` until [parallel-drafts](../parallel-drafts/design.md).
