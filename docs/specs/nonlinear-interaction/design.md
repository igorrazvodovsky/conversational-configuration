...
The draft assumed reopening a thread "just works" over LangGraph checkpoints. Verification proved otherwise: CopilotKit v2 (1.65.0) switches the active thread but **never fetches its history** — the runtime exposes `GET /api/copilotkit/threads/{id}/messages` and `/threads/{id}/state`, yet no client code calls them, so a reopened thread rendered empty. The hand-rolled hydration that closed the gap now lives in `useWorkspaceAttachment` (`src/hooks/use-workspace-attachment.ts`, mounted in `src/app/workspaces/[id]/page.tsx`), which also changed what resumption means: messages still hydrate from the thread, but `configuration` hydrates from the durable *workspace* — the thread checkpoint is a historical record, and the workspace spec's design documents two further traps (the connect delivering checkpoint state after the seed; the agent instance swapping identity after the initial connect). Three traps discovered here and still handled there: the runtime returns LangChain-style tool calls (`{id, name, args}`) which must be converted to the AG-UI shape (`{id, type, function: {name, arguments}}`) or `setMessages` throws; thread switching fires a `connect` that can wipe the store, so hydration waits for the agent to settle and briefly re-applies if wiped; and switching clears the agent's *messages* but leaves the previous thread's `configuration` on `agent.state`.

That last one is not cosmetic. AG-UI's `prepareRunAgentInput` sends `agent.state` as every run's initial state, so whatever configuration the frontend is holding seeds the thread the next message runs against — a new conversation opened from a thread that has choices starts from those choices and persists them into its own checkpoint. The hook therefore calls `agent.setState({})` on each thread change, before anything can be sent; hydration puts the real state back for a thread that has one.

It clears rather than writing an empty `Configuration`: the agent's `empty_configuration()` derives `statuses` from the solver, and a client-authored `{choices: {}, statuses: {}}` is truthy, so it would beat the `or empty_configuration()` fallback in `configuration.py` and leave the canvas with no option statuses. Clearing is safe for a thread that already has a saved agreement — the adapter builds the run input by *picking* keys off the frontend state, so an empty state yields an input with no `configuration` key, and LangGraph leaves channels absent from the input at their checkpointed values (verified against the running server, not just read).

Message hydration is display-only — LangGraph resumes runs from its own checkpoint regardless. State hydration is not: what the frontend holds is what the next run starts from.
