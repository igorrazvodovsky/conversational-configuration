...
- Abandoning a repair is a tool call that cannot move anything. The prototype walkthrough (2026-08-20) found the model answering `abandonMessage` with `undo_change` in two of four samples — reversing the change *before* the one the customer declined, taking the priced candidate with it, and once reporting the prior priced configuration as restored while the sheet showed no price. Prompt wording alone could not be verified (two of the four samples were already clean), so the fix is structural: `keep_as_is` holds no `configuration` in its update and writes no history entry, routed through `_reply` rather than `_committed`, and the prompt maps the abandon sentence onto it and nothing else. A unit test asserts the update carries messages only, which is an assertion about what the tool *can* do rather than about what the model chose this time. The scenario check widened with it: `abandon_changes_nothing` compared recorded choices, which the failure left untouched, and now compares the priced whole too and asserts that no state-changing tool was called at all ([conversation checks](../conversation-checks/design.md)).
- Applying a repair moves the fee rather than removing it: `_repriced` (see the [agent-tools design](../agent-tools/design.md)) completes again inside the same batch, so the walkthrough's "the fee moved with it" is now what happens — the same walkthrough had found the sheet unpriced after every repair, with half its terms back at "not yet decided".
