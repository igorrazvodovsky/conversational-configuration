...
Serves discovery principle [the agent proposes and the user decides](../../discovery/principles/agent-proposes-user-decides.md), whose test is *can the user always tell who chose a value*. Provenance is carried by a status vocabulary — `you`, `agent`, `auto`, `proposed` — that has to read the same everywhere it appears. It is also constrained by [every refusal names the rules that caused it](../../discovery/principles/refusals-name-their-rules.md): unavailable options explain themselves through a native `title` on a disabled control, and no restyling may cost the user that reason.
...
*Out:* the chat pane's own composition, which belongs to the [chat pane spec](../chat-pane/requirements.md). This spec supplies the primitives it uses and the identity it matches, and nothing more. Also out: the dead starter code listed in `CLAUDE.md`, and with it the `[data-copilotkit]` suggestion-pill highlight rules in `globals.css`, which serve `use-example-suggestions.tsx` alone.
...
- GIVEN a new UI element, WHEN I need a button, badge, card, panel or disclosure, THEN it comes from `src/components/ui/` installed by the shadcn CLI, rather than from a new `className` string.
...
- GIVEN any in-scope surface, WHEN it is inspected, THEN no element has a rounded corner, including ones whose radius came from a literal class rather than the radius token. Only the dead starter files are exempt, and they keep their literal `rounded-*` classes.
...
- GIVEN the agent is running or a card has gone stale, WHEN controls are shown, THEN they are disabled or inert exactly as before.
- GIVEN `src/components/ui/`, WHEN its files are inspected, THEN they are current upstream shadcn source in the Lyra style, rather than hand-written approximations.
