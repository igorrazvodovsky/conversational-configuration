...
The fix is a reload on the transition of `agent.isRunning` from true to false, calling `copilotkit.reloadSuggestions(agent.agentId)` — the public counterpart of the `clearSuggestions(agent.agentId)` the run's start called, on the same key, guarded the same way for an agent with no id. On a run that completed normally this is a second reload of the same static list, arriving beside the library's own. It should be invisible: `reloadSuggestions` clears and re-adds a static config synchronously, so both notifications fall inside one React update. *That is reasoning, not observation*, and it is the kind of claim only looking settles — the verification record below records it as outstanding.
...
- Across a run the strip is absent, and returns once when a run *completes*: the library renders the view only while idle, and the frozen catalogue means what returns is computed from settled state. A run that ends any other way is decision 8's subject and is unchecked — see below.
...
*Three checks are outstanding, all of them from decisions 7 and 8.* The session that wrote those decisions could not run them: the repository was being edited from another session at the time, so what the app was serving was not the tree under test. They are the checks that discriminate, and none of the reasoning above substitutes for them.

- Stop a reply mid-stream on a workspace whose strip is *not* empty — a candidate standing, so the trade-off pair is offered — and confirm the pills come back when the run ends. This is decision 8's whole subject; a workspace with nothing recorded cannot show it, because an empty strip there is the correct answer either way.
- Watch the end of a reply that completes normally, on the same workspace, for a blink as the two reloads land.
- Ask an *ask why* pill's question, and confirm that when the reply lands the strip offers the next unexplained term rather than the same question again — and that asking the traffic hypothetical retires that pill while the usage profile stands.
