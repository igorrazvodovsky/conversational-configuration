...
- As a presenter running a scripted demo, an empty workspace still opens with the entry prompts the scenarios start from, each worded as it has always been.
...
- GIVEN a workspace where nothing has been said and nothing recorded, WHEN the strip renders, THEN it shows the entry prompts, every one the demo scenarios open from unchanged in wording. The set may gain a prompt for an entrance the customer has no other way of discovering; it may not lose one or reword one. *Both tests, not just the second:* a customer who has described the building and been asked a question back has recorded nothing, and offering them a Munich hotel is this spec's own opening complaint arriving one turn in rather than three revisions in. Between the first message and the first recorded choice the strip is empty, which the last criterion here already calls correct.
...
- GIVEN an untouched workspace, WHEN the strip renders, THEN handing over the customer's own requirements document is among the moves it offers. It is a move of the user inventory like any other ([Conversation moves](../../discovery/models/Conversation%20moves.md) §2), it is the [delivery lead](../../discovery/jtbd/persona-delivery-lead.md)'s ordinary way in rather than an advanced one, and nothing else in the interface says it is possible.
