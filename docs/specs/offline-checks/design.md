...
Not checked: every component, every hook, and `card-dispatch.ts` — which is a hook, and whose interesting behavior (a card going inert, a dispatch racing a run) is behavior of the CopilotKit core it calls. Constitution #9 keeps UI on the running app, and the hydration rules in `docs/specs/chat-surface/design.md` are the kind of thing only a browser shows. That last clause has since been acted on: a browser now shows it on every push, in the [hydration checks](../hydration-checks/design.md), which are a separate runner for the reason decision 1 gives here — the absence of a DOM is this suite's point. The decision *not* to render components and hooks was re-taken there rather than inherited, and it stands.
...
- No check here reaches a React component or a hook — by design, re-argued in the [hydration checks](../hydration-checks/design.md) §1. The hydration half of this gap is closed: the traps in `docs/specs/chat-surface/design.md` and `docs/specs/agreement-document/design.md` now fail a check rather than a console. What those specs say about *layout* — a panel's geometry, a scroll offset surviving a mode switch — is still found only by running the app.
