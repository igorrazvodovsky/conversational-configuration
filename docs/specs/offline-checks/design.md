...
One check reads source rather than calling it. `tests/legibility.test.ts` holds the two mechanical rules of constitution #16 — no live source sets a text size below 12px, and no hand-written element carries an unconditional `opacity-*` beside a text token — because a Tailwind class is not a computed style until a browser has both stylesheets, and neither vitest project has one. So the rule is about source and is read from source, the way the prompt assertions in `couplings.test.ts` already are. It skips `components/ui/`, which is installed rather than written, and the starter code the gotchas enumerate.
...
`agent/tests/grammar_dump.py` builds every sentence from the agent's own helpers, for a set of inputs it names itself, and prints the lot as JSON, along with the openers the tool layer refuses and the prose it must let through ([one-gesture-one-action](../one-gesture-one-action/design.md)), so the set they cover is asserted in both directions — exactly the two sentences that set a value, and no reconciliation or draft move. The frontend's own copy of that predicate, `isGesture()`, is run over the same sentences and the same prose in the same check: it decides which messages the transcript draws no row for ([agreement-document](../agreement-document/design.md)), so a rule that grew on one side and not the other would hide a customer's typed message. `tests/couplings.test.ts` runs it through the shared loader in `tests/agent-dump.ts` — shared with the [interface checks](../interface-checks/design.md), which take their agreement payloads from the same dump — builds the same sentences from the same inputs, read out of the dump so the two provably compared the same thing, and asserts the two objects equal. Comparing the two sources as text, which is what this check did first, passes on two files that hold the same words and build different sentences. This doesn't.
