...
- [Demo scenarios](../demo-scenarios/requirements.md) (still draft): the four scenarios are to be recast to the job stories in `docs/discovery/jtbd/job-stories.md` before that spec is approved.
- [Environmental footprint](../environmental-footprint/requirements.md) (draft): shares the `usage_profile` variable, introduced here. Once footprint lands, the agreement comparison carries a footprint delta beside the monthly-price delta ([trade-offs are shown as a pair](../../discovery/principles/trade-offs-shown-as-a-pair.md)), and the first proposal's objective can no longer default silently to cheapest.
