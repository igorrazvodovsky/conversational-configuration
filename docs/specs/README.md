...
| [remembered-split](remembered-split/) | The division between canvas and chat that the operator drags survives the reload and travels between workspaces, read from the request so the server and the first client render agree | Implemented |
