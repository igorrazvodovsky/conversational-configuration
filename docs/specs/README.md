...
| [agent-tools](agent-tools/) | Configuration state + solver-backed agent tools | Implemented; the two gaps the 2026-08-20 walkthrough found are closed — a batch now records what fits and names what it declines, and every unavailable option carries its rules in state |
| [nonlinear-interaction](nonlinear-interaction/) | Revision with ripple, parallel candidates, resumption | Implemented; the parallel-candidates half now lives in [parallel-drafts](parallel-drafts/). Two defects from the 2026-08-20 walkthrough fixed: abandoning a repair is now a tool that cannot move state, and applying one reprices instead of leaving the agreement unpriced |
| [conversation-checks](conversation-checks/) | The [demo scenarios](../discovery/scenarios/) as executable checks on conversation behavior | Implemented; all five run against the live agent. The scenarios themselves are discovery, and the presenter document is [docs/demo-scenarios.md](../demo-scenarios.md), walked in a browser 2026-08-20 and its findings fixed. The abandon assertion, which read recorded choices and so missed the defect it was written for, now compares the priced whole as well — the checks have not been re-billed since |
| [offline-checks](offline-checks/) | Everything that runs without a provider key: the agent's tools and store, the frontend logic that is not UI, the couplings that cross the language boundary, and the CI that runs them | Implemented 2026-08-20. The grammar is compared by building it on both sides, and the system prompt now has a rule for every element of it — that prompt edit is the one thing here not run against the live agent, which needs the conversation checks |
...
| [agreement-document](agreement-document/) | The canvas whole: the agreement as a layered document — recitals, operative terms, schedules — the edit grammar every value on it uses, and the in-chat generated controls that share that grammar | Implemented; the move mix the genre change predicts is not yet measured. The 2026-08-20 walkthrough's two gaps are closed: a decided term offers its alternatives again, and an unavailable option names the rules that rule it out |
