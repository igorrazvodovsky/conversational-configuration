...
- *History is per draft and crosses conversations.* Undo must survive what the agreement survives, so history lives in the workspace store beside the configuration it belongs to; thread checkpoints have the wrong scope (the [agreement-workspace spec](../agreement-workspace/requirements.md) demoted them to historical records). The most recent applied batch may have landed from another conversation; undo reverses it regardless — the same last-write-wins stance the store already takes for writing. Narrowed from workspace-level by [parallel-drafts](../parallel-drafts/requirements.md) (implemented 2026-08-17): each draft keeps its own history, because a switch between drafts is not a change to a document and an undo taken after one would otherwise restore another document's state. A fork starts with no history rather than a copy of its source's. Each draft's history stays linear — what branches is the document, not the history.
- *History is per-batch snapshots of the configuration, not a move log.* A log of inverse operations would need an inverse per tool where a snapshot needs one hook at the store's single write-through point (constitution #10). This decision was originally argued against frames, whose lossiness made them unfit to restore a prior state; [parallel-drafts](../parallel-drafts/requirements.md) removed frames, and a draft is a whole configuration, so nothing about a draft tempts a ride-along either — the two mechanisms answer different questions, one about this document's past and one about a document beside it.
