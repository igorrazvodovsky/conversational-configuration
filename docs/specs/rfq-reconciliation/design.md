...
- The document badge is a sibling of the collapsible trigger, never a child. It is itself a popover trigger, and a button inside a button is invalid HTML that fails hydration on every load — a cousin of the trap the [chat-surface design](../chat-surface/design.md) records.
