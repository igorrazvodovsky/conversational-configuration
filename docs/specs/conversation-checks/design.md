...
*How much the steering turns actually flap, measured 2026-08-22.* Five runs of the suite across two trees — one standalone, two comparisons — failed a different assertion nearly every time and shared none. Every repeated result was a dispatched turn; every one-off was a judgment turn, on both trees. The practical consequence for reading a report: a single run tells you almost nothing about one assertion, and a difference between two arms is only evidence when the assertion belongs to the state-critical class or the same difference survives a second run. One exception is on record and unexplained — `revising_is_one_reconcile_call` is dispatched and still failed once in three ([one-gesture-one-action](../one-gesture-one-action/design.md)) — so near-deterministic is not the same as deterministic, and a lone movement there deserves a re-run rather than a verdict.
...
A 1000 kg car makes the statement collide, and the 630 kg one is kept for a different reason than it was chosen for. The turn that states the shaft is prose, so which action it reaches is the agent's judgment, and a collision there would test that judgment instead of what the renewal scenario is for — an unmediated entry point landing on the right variable at all. [Mid-contract revision](../../discovery/scenarios/mid-contract-revision.md) already exercises the repair path from a typed revision, and the [one-gesture-one-action](../one-gesture-one-action/design.md) checks exercise it from a dispatched one. The conditional branch stays in the scenario, so a model change that makes the statement collide is still asserted rather than skipped.
...
The same rule binds what the assertions may assume about the *record*. Both arms run this tree's assertions against the other tree's agent, so an assertion that indexes a key only the current shape has crashes the older arm and takes the whole comparison with it — not a weaker result but no result. Reading the document's clauses goes through one helper that accepts either shape, the single list of [document-clauses](../document-clauses/design.md) and the `requirements`/`unmapped` pair that preceded it. It is a view for reading rather than a lift: no identity is minted, because every assertion reads the citation, the quote, the variable, the value and the mark, and both shapes carry all five. This is the rule `report_trace` already follows for a ref predating the log, generalised — the harness reads what the older tree can say, and the comparison is about what moved rather than about which shape said it.
...
Each arm also reports its run as the agreement's own record holds it: the actions taken on every draft, in order, read from the store rather than from the turns ([action-log](../action-log/design.md)). The report prints the two sequences and names the action they diverge at, which is often one action earlier than the assertion that noticed. Forward only — an arm whose tree predates the log reports nothing, and the report says which side that was rather than reading the silence as agreement.

Collecting it is the harness's job and not each scenario's: `Conversation` registers itself as it opens, `run` clears the register per scenario and reads the workspaces afterwards. A scenario that returns early — several do, when a check that later ones depend on fails — reports its trace anyway, which a per-scenario call at the end would not.
...
Verified 2026-08-19: `uv run pytest` passes the unit suite and deselects the scenarios, and `uv run --env-file ../.env pytest -m scenario` passes all five against the live agent in one process, in a little over two minutes. Comparison mode was verified against one scenario on 2026-08-14 — `python tests/compare_refs.py HEAD` creates and cleans its worktree, runs both arms sequentially, and reported every assertion equal across the two trees, reproducing through the harness the conclusion the throwaway A/B reached about the de-duplicated prompt.

Run over the full sweep for the first time on 2026-08-22, which is what found the shape assumption above. One scenario had never been comparison-run before, and its assertions indexed `rfq["clauses"]`, so the baseline arm died on a `KeyError` and the whole comparison returned nothing — the failure mode the sequential design exists to avoid, arriving from a direction nobody had checked. Restricting the earlier verification to one scenario is what hid it: a harness that runs against older trees has to be exercised over every scenario it will run, because each one reads the record its own way.
...
A third, closed for gestures on 2026-08-21: a conflicting `set_choices` on a variable the conversation hadn't yet decided is refused with its rules rather than answered with repair options. [One gesture, one action](../one-gesture-one-action/design.md) routes every dispatched `Set …` sentence to `revise_choices`, so a click or a sheet edit reaches repairs whether or not the term was decided. It stays open for prose, where the agent still picks the action, and that is the case the renewal scenario's 630 kg car steps around rather than papers over.
