...
- Frontend files: `src/lib/configurator.ts` (types, model display data, `resolveValue`, the layer mapping, `optionNote`, `monthlyDelta`, the message builders), `src/components/config-canvas/` — `index.tsx` (shell: header, register summary, draft switcher, routed dispatch, overlay), `draft-switcher.tsx` ([parallel-drafts](../parallel-drafts/design.md)), `document-parts.tsx` (the shared `DocumentView`, `OptionEditor`, `ValueToken`, `Gloss`, `ProvenanceBadge`, `DeviationMark`, `Clause`), `recitals.tsx`, `terms.tsx`, `schedules.tsx` (the three layers share their heading as the `LAYER_HEADING` class string from `document-parts.tsx`, never as a component — see below); `src/components/generative-ui/ask-choices.tsx`, over the shell every in-chat card shares (`card-shell.tsx`, described in the [nonlinear-interaction design](../nonlinear-interaction/design.md)); `src/hooks/use-configurator-ui.tsx`.
...
- The three layer headings are identical markup, and factoring them into a `<LayerHeading>` component breaks hydration on every load. Measured, not reasoned: with the component in place the schedules' Radix collapsibles hydrate with different `useId` values than the server sent, on a cold build, on every reload; reverting it alone makes the page clean again. A component is a fiber, and a fiber inserted above those collapsibles moves the tree path the ids derive from — the same page-wide `useId` hazard the [chat surface](../chat-surface/design.md) records for menus in the hydrated tree, reached from a direction that looks like ordinary tidying rather than like adding a widget. The duplication is shared as a class string (`LAYER_HEADING`) instead, which costs the same three `<h2>` elements and changes no tree shape. The draft switcher reached the same hazard from a third direction ([parallel-drafts](../parallel-drafts/design.md)): a node that appears only on the client, because the canvas hydrates after the workspace seed has landed and the server had no drafts to name. It renders nothing until its own mount effect has run.
