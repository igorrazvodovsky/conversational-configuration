...
- Frontend files: `src/lib/configurator.ts` (types, model display data, `resolveValue`, the layer mapping, `optionNote`, `monthlyDelta`, the message builders), `src/components/config-canvas/` — `index.tsx` (shell: header, register summary, frames strip, routed dispatch, overlay), `document-parts.tsx` (the shared `DocumentView`, `OptionEditor`, `ValueToken`, `Gloss`, `ProvenanceBadge`, `DeviationMark`, `Clause`), `recitals.tsx`, `terms.tsx`, `schedules.tsx` (the three layers share their heading as the `LAYER_HEADING` class string from `document-parts.tsx`, never as a component — see below); `src/components/generative-ui/ask-choices.tsx`, over the shell every in-chat card shares (`card-shell.tsx`, described in the [nonlinear-interaction design](../nonlinear-interaction/design.md)); `src/hooks/use-configurator-ui.tsx`.
