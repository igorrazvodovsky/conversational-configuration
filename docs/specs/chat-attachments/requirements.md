...
Before this spec, the chat composer offered a paperclip and the file it took went nowhere. Every non-image attachment was delivered to the model as an image and the run failed with `invalid_image_format`. The rejected content also persisted in the thread, so the conversation stayed broken for every message after it. This spec makes an attached file arrive as something the agent can read, and makes every file it can't read fail in the composer rather than in the run.
...
Serves discovery principle [configuration can start from any variable, in any order](../../discovery/principles/start-from-any-variable.md), whose limiting case is an inbound document. An entry point that errors when used is not an entry point, and the paperclip advertised one that didn't work.
...
- As a user attaching something the agent can't read, I am told which file and why before I send, and I send my message without it.
...
- GIVEN a file whose type isn't accepted, or one over the size cap, WHEN the user picks, drops or pastes it, THEN it is refused in the composer with a visible message naming the file and the reason, and no run is started with it.
- GIVEN an image attachment, WHEN the message is sent, THEN it reaches the model as an image exactly as it did before, because the guard must not cost the one modality that already works.
- GIVEN a message that carries media the model would reject anyway — a thread saved before this spec, a type that slipped the composer — WHEN the agent runs, THEN that content is degraded to a text placeholder naming the file and the run completes. No attachment may fail a run.
...
- [RFQ reconciliation](../rfq-reconciliation/requirements.md) owns what an inbound requirements document does. Its ingestion is specified for pasted text, and this spec is the file equivalent of that paste. Whether an attached file becomes a supported RFQ entrance — with document provenance and a deviation register — is that spec's decision rather than this one's. Until it lands, an attached document reads as ordinary conversation.
...
- [Agent tools](../agent-tools/requirements.md) needs no new tool and no state change. A file arrives as message content and is acted on through the existing tools, so provenance, ripple and staleness behave as they already do.

## Out of scope

PDF, Office and OCR extraction. Storing, re-serving or re-attaching a file. Showing attachments on the canvas. The agent asking for a file. Reading an image as data — dimensions, model numbers — rather than as an image. And anything the [RFQ reconciliation spec](../rfq-reconciliation/requirements.md) claims: extraction into commitments, document provenance, the deviation register.
