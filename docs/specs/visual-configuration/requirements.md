...
- [demo-scenarios](../demo-scenarios/requirements.md): the render asserts nothing the harness can read — it makes no tool call and changes no state. Verified by running the app (constitution #9). The cabin-height rule is assertable and may be worth a turn in an existing scenario.
