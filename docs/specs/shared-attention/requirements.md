...
The document is three layers deep with its schedules collapsed, and before this spec the agent couldn't see any of it. It didn't know which value the operator had an editor open on, and it didn't know whether the term it had just changed was on screen or two screens down. It answered as though the agreement were one flat sheet, always fully visible.

The gap runs both ways. When a revision forces three hardware values inside a collapsed schedule, the change lands where nobody is looking, and the only way the agent could report it was to say in chat what the canvas already shows, which the conversation move inventory lists among the agent's never-moves. When the operator asks "why is this one greyed out?" with an editor open, the agent had to guess which one, or ask.
...
Serves discovery principle [the canvas holds the state and the chat explains it](../../discovery/principles/canvas-holds-state-chat-explains.md): the principle's division of labour only holds if what the canvas shows is actually visible, and pointing at a changed value is the alternative to restating it in chat. Serves [configuration can start from any variable, in any order](../../discovery/principles/start-from-any-variable.md) in its non-verbal form, starting from the value you are pointing at without having to name it. And it puts [showing the ripple at the moment of revision makes nonlinear change workable](../../discovery/assertions/ripple-at-the-moment-of-revision.md) properly under test, because frames F3 and F5 of the [ripple storyboard](../../discovery/models/Ripple%20storyboard.md) place the pending revision and its repairs on the document, and a document-side artifact below the fold isn't showing the ripple at the moment of revision.
...
- As an operator who has deliberately collapsed a schedule, it stays collapsed. What I have chosen to put away, the tool doesn't reopen.
- As an operator reopening an old conversation the agreement has moved past, the agent knows the transcript above is historical and doesn't reason from it as though it were current.
...
- GIVEN the operator opens or closes an editor, WHEN nothing else happens, THEN no agent run starts. This channel is read on the next turn the user causes, and is never a trigger.
- GIVEN the agent answers a question containing a demonstrative — "this one", "that clause" — WHEN it can resolve it from the open editor, THEN it does; WHEN it can't, THEN it asks which value is meant rather than picking one.

What the document may do:

- GIVEN the agent's turn changes or proposes one or more values, WHEN the run ends, THEN the document brings the affected values into view if they aren't already, expanding the schedule that holds them, and marks them transiently so they can be found on arrival.
- GIVEN several values changed, WHEN the reveal happens, THEN the view goes to the topmost affected value in document order that isn't already in view, and the rest are marked in place. The document doesn't tour them.
...
- GIVEN the agent's turn changed nothing, THEN nothing moves. Directing attention is a consequence of a move, never a move of its own, and an agent that wants the operator to look somewhere without changing anything says so in words.
- GIVEN a reveal, THEN it isn't a move: it doesn't appear in the transcript, doesn't enter the undo history, and leaves no trace in state once the mark fades.

## Discovery amendment — applied

The [conversation move inventory](../../discovery/models/Conversation%20moves.md), *Agent moves*, had no move for directing the operator's attention, and its never-moves forbid restating in chat what the canvas already shows. Between them, the agent had no legal way to point at a value below the fold. On approval of this spec, *Agent moves* gained one row — *reveal a change it just made*, unasked, bounded to values its turn changed, with no effect on the document beyond what is visible — and the open edge about the returning operator, under *Edges this model does not settle*, notes that the read half of this channel is the first signal a session map has to work with.

## Relationship to other specs

- [Agreement document](../agreement-document/requirements.md) owns the three layers, the schedules' disclosure, and the option editor this spec reads and moves. The precedent is already there: a schedule holding an unanswered deviation opens itself, on the argument that a deviation the operator can't see is one they can't answer. Revealing a change is the same argument applied to the agent's own moves.
...
- *Highlighting as an explanatory device* — colouring the ripple's causal chain, drawing lines between a term and the hardware it forced. That is ripple-disclosure work and belongs with the storyboard's open question rather than here.
- *Presence between users.* One operator per screen, and nothing here is about seeing where someone else is looking.
