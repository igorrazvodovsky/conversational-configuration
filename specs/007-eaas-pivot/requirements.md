...
- 001 (product model): extended with service variables, coupling constraints, and monthly pricing data; the capex price field is replaced, not kept alongside.
- 003 (agent tools) and 004 (canvas): elicitation prompt and canvas grouping updated; tool surface expected to be unchanged.
- 006 (demo scenarios, still draft): the four scenarios are recast to the job stories in docs/discovery/jtbd/job-stories.md before 006 is approved.
