...
# The inputs both sides build from, named here so the two cannot drift apart.
...
# half to even, JavaScript's toLocaleString rounds half away from zero.
...
# A dispatched message is nothing but its lines. One of the customer's own is
# prose.
...
def configurations() -> dict:
    """Agreements the agent actually built, so the frontend's invariants can be read
    off them rather than asserted from memory. A fixture assembled by hand can be
    valid in shape and impossible in fact.
    """
...
def log_fixture() -> list[dict]:
    """Holds every case the counts turn on, and more reversible entries than the
    cursor may walk back through. The invariant it keeps is the store's own: every
    applied or abandoned entry precedes every reversed one.
    """
...
def record_shapes() -> dict:
    """Built rather than read from the source, so an optional key added by a later
    write is in it. Runs against a temp store: a check must never write into the
    developer's own agreements.
    """
...
# One action, so an entry's own keys are built rather than described. Without it
# a key added to an entry would be a durable record key nothing accounts for.
...
# Restored, not cleared: a caller that set its own override must get it back.
            if outer is None:
...
# Which of those sentences `set_choices` refuses, run through the real predicate
...
# The prose itself, because the frontend runs the same predicate to decide which
# messages the transcript draws no row for.
        "prose": NOT_GESTURES,
# The keys a configuration declares, and the keys one the agent built carries:
# an optional field is in the first and not the second.
...
# A mark is optional on a clause, so the Literal sits inside a NotRequired.
...
# The population the ontology has to name (constitution #15). The ontology is
# prose and can only be read, so what crosses is what it has to account for.
        "toolNames": sorted(t.name for t in configuration_tools),
# Every name an entry's log may carry, and how far the cursor may walk back.
# Both without a run, so the check can assert the set rather than a sample.
